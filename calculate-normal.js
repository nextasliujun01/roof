...
const short_eave_side_offset = 4;
...
var g_no_metal_on_vert_edge = false; //User can select if use metal shingle next to the vertical edge or not, 
//if there is one.
...
function test() {
    ...
    console.log("cv_real height " + cv_real_height);
...
function check_short_offset(roof) {
    ...
    if (g_normal_trapezoidal) {
        ...
        let h = ss_height + eave_ss_offset;
        if (g_left_ref_line) { // left offset is shorter than right offset
            ...
            let x = compare_offset_with_shingle_width(roof, roof.left_offset, cv_left_gap_x, h);
            console.log("1. true or false " + x);
            return x;
        }
        else { // right offset is shorter... 
            let x = compare_offset_with_shingle_width(roof, roof.right_offset, cv_right_gap_x, h);
            console.log("2. true or false " + x);
            return x;
        }
    }
    else { // inverted trapezoidal/triangle
        let  h = ss_height + ridge_ss_offset;
        if (g_left_ref_line) {
            let x = compare_offset_with_shingle_width(roof, roof.left_offset, cv_left_gap_x, h);
            return x;
        }
...
function find_non_symm_trapezoidal_ss(roof) {
    ...
    if (g_left_ref_line) {
        let start_y = eave_ss_offset;
        if (g_normal_trapezoidal) {
            // Process rows: 
            for (let row = 0; row < cv_total_rows; row++) {
                let y1 = (start_y + row * ss_height); // bottom of SS (level N + 1) in document.
                let y2 = (start_y + (row + 1) * ss_height); // top of SS (level N + 1)
                let next_top = (start_y + (row + 1) * ss_height + ss_height); // top of next level (level N+2).
                let row_width_next = find_width_of_SS(roof, roof.eave, next_top, cv_left_gap_x, cv_right_gap_x);
                let ss_per_row = Math.floor(row_width_next / ss_width);

                //var ss_per_row = find_normal_ss_per_row(roof, start_y, row, y1, y2, cv_left_gap_x, cv_right_gap_x);
                //var ss_length = ss_per_row * ss_width;
                let start_x = cv_ref_line_x;
                get_ss_per_row(ss_per_row, ss_width, start_x, y1, y2);
                if (row == 0) {
                    cv_max_ss_in_row = ss_per_row;
                }
            ...
            console.log("total rows" + cv_total_rows);
            for (let row = 0; row < cv_total_rows; row++) {
                let y1 = (start_y + row * ss_height); // use this y1.
                let y2 = (start_y + row * ss_height + ss_height);

                // at y=y1, check the width of SS area. x/h = offset/real_height;
                let row_width = find_width_of_SS(roof, roof.ridge, y1, cv_left_gap_x, cv_right_gap_x);
                console.log("row length is " + row_width);
                let ss_per_row = Math.floor(row_width / ss_width);
        ...
        if (g_normal_trapezoidal) {
            ...
            for (let row = 0; row < cv_total_rows; row++) {
                ...
                let y2 = (start_y + row * ss_height + ss_height);
                let next_top = (start_y + (row + 1) * ss_height + ss_height); // top of next level (level N+2).
                //let ss_per_row = find_normal_ss_per_row(roof, start_y, row, y1, y2, cv_left_gap_x, cv_right_gap_x);
                // at y=y2, check the width of SS area. x/h = offset/real_height;
                let row_width = find_width_of_SS(roof, roof.eave, next_top, cv_left_gap_x, cv_right_gap_x);
                let ss_per_row = Math.floor(row_width / ss_width);

                // total ss length
                let ss_length = ss_per_row * ss_width;
                let start_x = cv_ref_line_x - ss_length; // count from right side!
                console.log("start x " + start_x);
                get_ss_per_row(ss_per_row, ss_width, start_x, y1, y2);
            ...
            for (let row = 0; row < cv_total_rows; row++) {
                ...
                // total ss length
                let ss_length = ss_per_row * ss_width;
                let start_x = cv_ref_line_x - ss_length;
                console.log("start x " + start_x);
                get_ss_per_row(ss_per_row, ss_width, start_x, y1, y2);
...
function find_width_of_SS_non_sym(roof, roof_width, y, left_gap_x, right_gap_x) {
    ...
    if (g_normal_trapezoidal) {
        if (g_left_ref_line) {
            ...
            let x_right = roof.right_offset * y / cv_real_height;
            ...
            let x_left = roof.left_offset * y / cv_real_height;
        ...
        // invert shape.
        if (g_left_ref_line) {
            let x_right = roof.right_offset * (cv_real_height - y) / cv_real_height;
            width_ss_row = roof_width - x_right - right_gap_x - cv_ref_line_x;
        }
        else { // reference line is on the right side:
            let x_left = roof.left_offset * (cv_real_height - y) / cv_real_height;
            width_ss_row = cv_ref_line_x - x_left - left_gap_x;
            console.log("y is " + y);
...
function get_num_ss_aligned_to_last_row(roof, roof_max_width, left_outside, right_outside, left_gap_x, right_gap_x) {
    ...
    if (g_normal_trapezoidal) {
        ...
        let current_num = Math.floor((end_x - shifted_x) / ss_width + 0.001);
...
function sec_get_num_ss_aligned_to_last_row(roof, roof_max_width, x_btm_left, x_top_left, x_btm_right, x_top_right) {
    ...
    if (g_normal_trapezoidal) {
        ...
        let gap1 = x_top_right - (start_x + ss_width);
        console.log("xxxxxx gap " + gap1);
        // if the current number of SS is 1, and last was 2, we may want to shift it by 1/2 width.
        // This part must be done with checking of end_x. 
        if ((num_ss == 1) && (last_row_info.num_ss == 2)) {
            if (start_x <= last_row_info.start_x) {
                let end = start_x + ss_width * 1.5;
                if (end < current_end_x) {
                    start_x = start_x + ss_width / 2;
                    let gap = Math.abs(start_x + ss_width - x_top_right);
...
function sec_calc_max_num_ss_first_row(roof, roof_max_width, x_btm_left, x_top_left, x_btm_right, x_top_right, invert) {
    // just check the length and try to fit
    if (invert == false) {
        let length = x_top_right - x_top_left - 2 * sec_min_short_hori_edge; // this must be > 0. 
        if (length < 0) {
            let start_x = 0;
            let total_num_ss = 0;
            return {
        ...
        let num = Math.floor(length / ss_width);
        let ss_len = num * ss_width;
        let start_x = 0;
        ...
        let length = x_btm_right - x_btm_left - 2 * sec_min_short_hori_edge; // this must be > 0. 
        if (length < 0) {
            let start_x = 0;
            let total_num_ss = 0;
            return {
...
function ms_fixed_width_normal(roof) {
    ...
    for (let row = 0; row < sec_cv_total_rows; row++) {
        ...
        let y1 = (start_y + row * ss_height); // bottom of SS (level N + 1) in document.
        let y2 = (start_y + row * ss_height + ss_height); // top of SS (level N + 1)
        ...
        //let row_width = find_width_of_SS(roof, roof.eave, next_top, cv_sec_left_gap_x, cv_sec_right_gap_x);
        let ss_per_row = Math.floor(usable_width / ss_width);
        ...
        if (g_normal_trapezoidal) {
            ...
            for (let row = 0; row < sec_cv_total_rows; row++) {
                ...
                if (left < sec_min_short_hori_edge) {
                    usable_width = usable_width - ss_width / 2; // remove 1/2 of ss_width from usable_width is a more conservative. 
                    shift_ss = true;
                }
...
function mix_ms_vertical_edge(roof) {
    ...
    if (g_normal_trapezoidal) {
        ...
        let sec_cv_total_rows = find_total_rows(cv_real_height, start_y, sec_ridge_ss_offset);
        console.log("total rows  " + sec_cv_total_rows); 
        // Process rows: 
        for (let row = 0; row < sec_cv_total_rows; row++) {
            let y1 = (start_y + row * ss_height); // bottom of SS (level N + 1) in document.
            ...
            let x_top_left = mix_left_edge_offset; // use this const is easy. 
            let [x_btm_right, x_top_right] = get_right_coord(roof, y1, y2);
                
            let usable_width = x_top_right - x_top_left - sec_min_short_hori_edge; // to put metal shingle, need at least 6 in width.
            // No need to consider shift 1/2 singles. 
            let ss_per_row = Math.floor(usable_width / ss_width);
            let start_x = mix_left_edge_offset;
        ...
        for (let row = 0; row < sec_cv_total_rows; row++) {
            ...
            let usable_width = x_btm_right - x_top_left - sec_min_short_hori_edge;
